...
3. **Cross-encoder rerank + a second gate** - a small local ONNX model (`RERANKER_MODEL`, default `Xenova/ms-marco-MiniLM-L-6-v2` via `fastembed`) scores each (question, chunk) pair directly, reorders the threshold-passed candidates by that judgment, and anything below `MIN_RERANK_SCORE` (default `-6.0`) is dropped as a second no-answer gate. This turned out to be necessary, not just a quality nicety: on short, structured chunk text ("Field: value | Field: value" rows), cosine similarity between a real embedding provider's vectors can stay misleadingly high for a genuinely unrelated question - confirmed empirically at 0.52 cosine (comfortably "positive") for a pair the reranker correctly scored -11.4. The threshold itself needs care in the other direction too: a genuinely relevant chunk can still score negative (`-2.41` was observed for a correct answer), so `MIN_RERANK_SCORE` was calibrated to `-6.0`, not `0.0`, to leave headroom above true irrelevance (around `-11`) without rejecting valid answers. Set `USE_RERANKER=false` to skip both (falls back to retrieval order, no second gate).
