...
### Overhauling logging (at explicit request)

- **Two rejection paths logged nothing at all.** `index_document`'s upload-too-large and embedding-model-mismatch checks raised before the method's own error handling even started, so a rejected upload left zero trace in the log. → Log `event=index_rejected` with the specific reason before raising, for both.
- **Routine, expected failures were logged identically to real crashes.** A corrupt/unsupported file upload (`DocumentParseError`) was caught by the same broad `except Exception: logger.exception(...)` as a genuine bug, producing a full stack trace for something that happens routinely. → Split into an expected branch (`WARNING`, no traceback) and an unexpected one (`ERROR`, full traceback via `.exception()`), applied consistently across the module.
- **No single line said whether a question was actually answered.** Only sub-step timing (retrieval, rerank) was logged; the final outcome wasn't. → Added one outcome log per exit path: `answer_no_answer` (with why) at every point the pipeline declines, `answer_success` (with source count, generator, whether the extractive fallback was used) on the successful path.
- **A real, previously-invisible inefficiency, found by the new logging itself, live in the running app.** The same session logged `event=session_start` three times in a row. Cause: `st.session_state.setdefault("service", RagService(...))` evaluates `RagService(...)` eagerly on every call regardless of whether `setdefault` stores it - and a Streamlit script reruns on every click, so this built (and threw away) a full `RagService` - LanceDB connection and all - on every interaction, not just the first. → Replaced with an explicit `if "service" not in st.session_state` guard. Would not have been noticed without the logging change.
