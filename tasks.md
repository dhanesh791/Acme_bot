...
Verified against the code on 2026-09-03. All nine gaps found in the review pass (G1-G9) were fixed the same day; see **Gaps found in verification review** below, each now marked Fixed with what changed. Since then: a local git repository was initialized and pushed to GitHub (`dhanesh791/Acme_bot`), the Streamlit UI was given Acme Retail branding, Word (`.docx`) support was added as an explicit scope addition (**§10**), the retrieval pipeline was rebuilt to a full RAG-standard design at explicit request (**§11**), fully-local answer generation was added as an opt-in mode at explicit request (**§12**), a second real rerank-threshold regression was found live and fixed, alongside a test-hermeticity gap it surfaced (**§13**), logging was overhauled at explicit request, surfacing a real app.py inefficiency (repeated `RagService` construction) along the way (**§14**), and per-answer confidence scores were added at explicit request, grounded in the retrieval pipeline's own reranker/cosine scores rather than a new heuristic (**§15**).

- `python -m pytest -q --basetemp=<short path>` -> **68 passed** (63 after §14 + 5 new confidence-score tests in §15: `_compute_confidence` unit coverage for both scoring paths and the gate-floor boundary, plus two `RagService.answer()` integration tests confirming the fields populate on a real answer and are `None` on a no-answer response). Of the 63: 57 after §13 + 6 new logging tests in §14 (session-start content, successful-index logging, corrupt-file logged as an expected warning not a traceback, oversized-upload rejection logged before raising, successful-answer outcome fields, no-answer-by-threshold logged at INFO not WARNING). Of the 57: 56 after local generation in §12 + 1 from §13's vague-meta-question regression test. Of the 56: 53 after the faithfulness-fallback UI notice + 3 from local generation in §12. Of the 53: 40 after Word support + 9 from the retrieval-pipeline rebuild (§11) + 1 the user added independently (`test_default_stack_filtered_excel_query_returns_evidence`, after finding and fixing the first `MIN_RERANK_SCORE` calibration bug themselves - see `ISSUES.md`) + 3 for the faithfulness-fallback notice.
...
## 14. Structured logging - error/failure/success/data, all at explicit request

Asked directly to "maintain logging - error logs, failure logs, successful logs, data etc." Audited every existing `logger.*` call site first rather than assuming coverage was already adequate - it wasn't.

- [x] **Found two rejection paths that produced no log entry at all.** `index_document`'s upload-too-large and embedding-model-mismatch checks both `raise ValueError(...)` *before* the method's own `try:` block even begins, so neither was ever logged - a user's upload could be silently rejected with zero trace. Fixed: both now log `event=index_rejected` at `WARNING` with the specific `reason=` before raising.
- [x] **Found routine, expected failures being logged as if they were system bugs.** `index_document`'s single broad `except Exception: logger.exception(...)` caught `DocumentParseError` - the ordinary, expected outcome of a user uploading a corrupt or unsupported file - and logged it with a full traceback via `.exception()`, indistinguishable in the log from an actual crash. Split into two branches: `DocumentParseError` -> `event=index_failure_expected`, `WARNING`, no traceback; anything else -> `event=index_failure_unexpected`, `ERROR`, full traceback. Same distinction applied to every other failure path in the module (`index_rejected`, `guardrail_refusal`, `embedding_mismatch`, `faithfulness_rejected`, `embedding_retry` all `WARNING`/no-traceback; `generation_failure`, `embedding_failure`, `fts_index_build_failed`, `fts_search_failed` all `ERROR`/traceback).
- [x] **Found `answer()` had no consolidated outcome log at all** - only sub-step timing (`retrieval`, `rerank`). Added `event=answer_no_answer` (with `reason=below_retrieval_threshold` / `below_rerank_threshold` / `llm_refusal`) at every point the method declines to answer, and `event=answer_success` (with `sources=`, `generator=`, `fallback=`, total `elapsed_ms`) on the successful path - so every question now has a single, greppable outcome line regardless of which of the pipeline's several decision points it exited from.
- [x] **Added `workspace=<session id>` to every log line tied to a browser session** (`RagService` now stores `self.workspace_id`, previously only used to build the LanceDB path and then discarded) - so `grep workspace=<id> data/app.log` gives one session's complete trace when multiple sessions are active. Added `event=session_start` logging the resolved embedding/reranker/generator providers.
- [x] **A real, previously-invisible inefficiency surfaced immediately by the new `session_start` log, live in the running app:** the same workspace logged `session_start` three times in a row for one browser session. Root cause in `app.py`: `st.session_state.setdefault("service", RagService(...))` - Python evaluates the `RagService(...)` argument *eagerly*, on every call, regardless of whether `setdefault` ends up storing it or not. Since a Streamlit script reruns top-to-bottom on every single click or keystroke, this constructed (and immediately discarded) a full throwaway `RagService` - LanceDB connection, provider selection, all of it - on every interaction, not just the first. Fixed with an explicit `if "service" not in st.session_state:` guard instead. Never would have been noticed without the logging change that revealed it.
- [x] Added 6 new tests using `caplog`: session start (workspace + providers present), successful index (`event=index_success`, `INFO`), corrupt-file rejection (`event=index_failure_expected`, `WARNING`, confirmed **no** `exc_info`/traceback), oversized-upload rejection logged *before* raising, successful answer (`sources=`/`fallback=` present), and no-answer-by-threshold logged at `INFO` (not `WARNING`) since insufficient evidence is a correct outcome, not a failure.
- [x] **Verified live, not just in unit tests:** restarted the app, uploaded a good file alongside a corrupt one, asked a question that triggered a faithfulness-gate rejection and fallback. Real `data/app.log` output confirmed every event, level, and field exactly as designed - including the full real trace of the fallback case (`faithfulness_rejected supported_ratio=0.41` immediately followed by `answer_success ... fallback=True elapsed_ms=15965`) - and confirmed `session_start` now fires exactly once per session after the `app.py` fix.
- [x] Added a "Logging" section to README.md (event/level table, field meanings, the expected-vs-unexpected distinction, `grep` patterns for session tracing). Full suite: 63 tests.

## 15. Confidence scores on answers, at explicit request

Asked directly to "add confidence scores to answers." Deliberately scoped to one specific meaning of "confidence" rather than the several plausible ones, and documented the choice rather than leaving it implicit.

- [x] **Decided what the score measures, and what it deliberately does not.** Confidence reflects the strength of the *best supporting evidence* the retrieval pipeline found - not the model's fluency, and not reduced when the faithfulness gate rejects a generated answer and falls back to the extractive one. An extractive fallback is the retrieved text verbatim, so it is exactly as grounded as a generated answer that passed the gate; conflating "did generation get rejected" with "how strong is the evidence" would have made the fallback case look artificially untrustworthy when it is, if anything, the safer of the two.
- [x] **Two scoring paths, chosen per the active pipeline, not one formula stretched to fit both.** When a real cross-encoder reranker is active, its score (an unbounded logit) is sigmoid-compressed centered on `MIN_RERANK_SCORE` (the same gate floor `answer()` already uses to reject weak evidence) - a candidate that barely cleared the gate reads as low confidence, a strongly positive match (specific factual matches score +4 to +8 in practice, see `ISSUES.md` §13) reads as high. Without a reranker (`NoopReranker`/hash provider, e.g. in tests), cosine similarity is linearly rescaled from `MIN_RETRIEVAL_SCORE` up to a perfect match instead, since a bounded cosine score doesn't need a sigmoid. Implemented as `_compute_confidence()` in `src/service.py`, taking the max over all retrieved evidence (not just the first citation) since any one strong chunk should carry the answer.
- [x] Labeled into three bands - High (>=70%), Medium (>=40%), Low (below 40%) - for a scannable UI badge, alongside the raw percentage for anyone who wants the number.
- [x] `ChatResponse` gained `confidence: float | None` and `confidence_label: str | None`, both `None` only for a no-answer response (confidence is meaningless when nothing was retrieved). Logged as `confidence=%.2f` in the existing `event=answer_success` line - no new log event needed.
- [x] Added a `.acme-confidence-badge` pill to the Streamlit UI (green/amber/red, distinct from the indigo citation pills so the two kinds of information aren't visually conflated), rendered in both the live-answer path and the chat-history replay path (the session-state message dict now carries `confidence`/`confidence_label`).
- [x] Added 5 tests: `_compute_confidence` directly against real project numbers (reranker-score path, cosine-fallback path, and the gate-floor boundary), plus two through `RagService.answer()` confirming the fields populate on a real answer and are `None` on a no-answer response. Full suite: 68 tests.
- [x] **Verified live via Playwright against the actually-running app**, using the brief's own example question ("What was South region's Q1 revenue and target attainment?") against `sales_q1.xlsx`: badge rendered as "🟢 High confidence (86%)", correctly persisted across chat-history replay after a second question, and correctly did **not** appear for a no-answer response - all three matching the design, not just the unit tests.
