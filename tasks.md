...
- `python -m pytest -q --basetemp=<short path>` -> **53 passed** (40 after Word support + 9 from the retrieval-pipeline rebuild in §11 + 1 the user added independently (`test_default_stack_filtered_excel_query_returns_evidence`, after finding and fixing the `MIN_RERANK_SCORE` calibration bug themselves - see `ISSUES.md`) + 3 for the faithfulness-fallback UI notice below).
